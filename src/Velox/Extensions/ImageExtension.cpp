...
/////////////////////////////////////////////////
    ...
    ImageExtension::ImageExtension()
            : VeloxExtension("Image Extension")
    {
        // Intentionally left empty.
    }

    void ImageExtension::InjectExtension(Interpreter &interpreter) const
    {
        ...
        imageType->AddItem("Clear", ExtensionFunctionOneParameter<Image>::CreateItem(
                [](ScriptContext &sc, Image &image, Item &param, unsigned int lineNumber) {
                    if (param.IsUndefined()) {
                        image.Clear(Color4d(0, 0, 0));
                    } else {
                        image.Clear(param.GetColorValue());
                    }
                    return Item::CreateUndefined();
                }
        ));
