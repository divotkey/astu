...
namespace velox {
    ...
    template<typename T>
    class ExtensionFunctionThreeParameter : public velox::InterpreterFunction {
    public:
        ...
        static std::shared_ptr<Item> CreateItem(Func func) {
            return Item::CreateFunction(std::make_shared<ExtensionFunctionThreeParameter>(func));
        }
         ...
         */
        ExtensionFunctionThreeParameter(Func func) : func(func) {
    ...
    protected:
        ...
        std::shared_ptr<Item> DoEvaluate(ScriptContext &sc, unsigned int lineNumber) final override {
            ...
            assert(exItem);
            return func(sc, *exItem, sc.GetItem("a"), sc.GetItem("b"), sc.GetItem("c"), lineNumber);
        }
